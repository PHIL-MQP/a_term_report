...
\section{Related Work} \label{related_work}
  ...
  \begin{itemize}
      ...
      \item Wirless network methods based on signal strength
  ...
  \subsection{Ultrasonic Mapping}
    ...
    Ultrasonic mapping (often referred to as sonar) was one of the first techniques used for indoor robot localization, and has been explored deeply since the 1980's. The most common approach is to use multiple emitter-reciever pairs placed around the perimeter of the robot, measure the range at each of those points, then localize to a given map of the environment \cite{drumheller_mobile_1987}. Alternately, some systems use one sensor and rotate it around to acheive the same effect \cite{leonard_mobile_1991, drumheller_mobile_1987}. The algorithms for interpereting the measured distances work by first extracting lines, then matching these lines to an existing map. Reported accuracies of the system in \cite{drumheller_mobile_1987} was 1 ft for position, and 10 degrees for angle. In \cite{drumheller_mobile_1987} and \cite{leonard_mobile_1991}, the reported rate of position updates is \SI{1}{\hertz}. Additionally, some methods will explicitely model the uncertainty of the position estiamte, or explicitely model the behavior of ultrasonic sensors to ignore unreliable data. A more recent and sophisticated approach to localizing with sonar can be found in \cite{tardos_robust_2002}, in which 24 sonar sensors in conjunction with encoders were used to perform simultaneous localization and mapping. Their experimental results report drifts of \SI{3.9}{\meter} and \ang{21} over the course of \SI{35}{\meter} of travel.
  ...
  \subsection{Cameras with Visual Tags}
    ...
    % Dorothy put research on cameras + tags here
  ...
  \subsection{Optical Flow}
    ...
    % Nicolette put resarch on optical flow (both mice and normal cameras here

\section{Evaluation of Localization Techniques} \label{methods}

  Each of the techniques presented thus far have strengths and weaknesses. In cases where those strengths and weaknesses are orthogonal, combinging multiple techniques improves the overall performance. This is the fundemental principle behind sensor fusion. For example, in \cite{kim_advanced_2008} the authors use a compass to make up for the inability of beacons to measure orientation of the robot. In order to handle all of the diverse challenges of localization in the FRC environment, it is necessary to combine techniques that work together. In this section we will outline the advantages and disadvantages of each technique and justify why none of the techniques discussed are sufficient on their own.
