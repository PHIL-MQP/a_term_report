...
\section{Related Work} \label{related_work}
  ...
  \subsection{Ultrasonic Mapping}
    ...
    Ultrasonic mapping (often referred to as sonar) was one of the first techniques used for indoor robot localization, and has been explored deeply since the 1980's. The most common approach is to use multiple emitter-reciever pairs placed around the perimeter of the robot, measure the range at each of those points, then localize to a given map of the environment \cite{}. Alternately, some systems use one sensor and rotate it around to acheive the same effect \cite{}. The algorithms for interpereting the measured distances work by first extracting lines, then matching these lines to an existing map. Additionally, some methods will explicitely model the uncertainty of the position estiamte, or explicitely model the behavior of ultrasonic sensors to ignore unreliable data. Reported accuracies of the system in \cite{} was 1 ft for position, and 10 degrees for angle. In \cite{} and \cite{}, the reported rate of position updates is \SI{1}{\hertz}.
